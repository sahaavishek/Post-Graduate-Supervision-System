...
router.get('/milestones/:student_id', authenticate, async (req, res) => {
  try {
    ...
    if (currentUser.role === 'student') {
      ...
      // Check access: either through students.supervisor_id OR supervisor_student table
      const hasDirectAccess = student[0].supervisor_id === supervisor[0].id;
      if (!hasDirectAccess) {
        // Check supervisor_student table for many-to-many relationship
...
router.post('/milestones', authenticate, [
  ...
  try {
    ...
    const { student_id, name, description, deliverables, due_date, progress } = req.body;
    const currentUser = req.user;
    ...
    const [result] = await pool.query(
      `INSERT INTO milestones (student_id, name, description, deliverables, due_date, progress, status)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [
        student_id,
        name,
...
router.delete('/milestones/:id', authenticate, async (req, res) => {
  try {
    ...
    // Check access (students cannot delete milestones)
    if (currentUser.role === 'student') {
      return res.status(403).json({ error: 'Students cannot delete milestones' });
    }
    ...
    if (currentUser.role === 'supervisor') {
      ...
      if (supervisor.length > 0) {
        ...
        if (student.length > 0 && student[0].supervisor_id !== supervisor[0].id) {
          ...
          const [relationship] = await pool.query(
            'SELECT * FROM supervisor_student WHERE supervisor_id = ? AND student_id = ?',
            [supervisor[0].id, milestones[0].student_id]
          );
          if (relationship.length === 0) {
            return res.status(403).json({ error: 'Access denied' });
          }
...
router.post('/tasks', authenticate, [
  ...
  try {
    ...
    // Immediately check if this task needs a deadline reminder notification
    // This ensures students get notified right away when supervisor sets/updates a deadline
    if (due_date) {
      try {
        const { checkSpecificTaskReminder } = require('../services/deadlineReminderService');
        // Run in background - don't block the response
        setImmediate(async () => {
          try {
            const result = await checkSpecificTaskReminder(taskId, student_id);
            if (result.notified) {
              console.log(`✅ Immediate reminder sent for task ${taskId}: ${result.message}`);
            } else {
              console.log(`ℹ️  No immediate reminder needed for task ${taskId}: ${result.reason}`);
            }
          } catch (error) {
            console.error('Error checking specific task reminder:', error.message);
          }
        });
      } catch (error) {
        // Don't fail the request if reminder check fails
        console.error('Error loading deadline reminder service:', error.message);
      }
    }
