...
router.post('/admin/create-user', authenticate, authorize('administrator'), async (req, res) => {
  try {
    ...
    try {
      ...
      res.json({ 
        message: `${role.charAt(0).toUpperCase() + role.slice(1)} account created successfully! A verification email has been sent.`,
        emailSent: true,
        user: {
          id: userId,
          name,
  ...
  try {
    ...
    if (users.length > 0 && users[0].avatar) {
      ...
      if (oldAvatarPath.startsWith('/uploads/avatars/')) {
        const fullPath = path.join(process.env.UPLOAD_DIR || './uploads', 'avatars', path.basename(oldAvatarPath));
