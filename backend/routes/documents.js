...
router.get('/:id', authenticate, async (req, res) => {
  try {
    ...
    // If supervisor is viewing a student's submission document, send notification
    if (currentUser.role === 'supervisor' && document.student_id && document.type === 'submission') {
      try {
        ...
        if (supervisor.length > 0) {
          const [student] = await pool.query(
            'SELECT user_id, supervisor_id FROM students WHERE id = ?',
            [document.student_id]
          );
          ...
          if (student.length > 0) {
            ...
            let hasAccess = false;
            if (student[0].supervisor_id === supervisor[0].id) {
              hasAccess = true;
            } else {
              ...
              hasAccess = relationship.length > 0;
            }
            
            if (hasAccess) {
              const studentUserId = student[0].user_id;
              
              // Check if notification was already sent for this specific document ID (within last hour)
              // This allows notifications for resubmissions (new document IDs) even if same week
              const [existingNotif] = await pool.query(
                ...
                [studentUserId, `%doc_id:${documentId}%`]
              );
              
              // Only send if no recent notification exists for this specific document
              if (existingNotif.length === 0) {
                // Check notification preferences
                const [prefs] = await pool.query(
                  'SELECT document_reviews FROM notification_preferences WHERE user_id = ?',
                  [studentUserId]
                );
                
                // Default to true if preferences don't exist (backward compatibility)
                const shouldNotify = prefs.length === 0 || prefs[0].document_reviews === 1;
                
                if (shouldNotify) {
                  // Get week number if available
                  const weekText = document.week_number 
                    ? `Week ${document.week_number}` 
                    : 'document';
                  
                  // Include document ID in message to track per-document notifications
                  await pool.query(
                    ...
                    [
                      ...
                      document.week_number 
                        ? `Your document Week ${document.week_number} is now being reviewed. [doc_id:${documentId}]`
                        : `Your document is now being reviewed. [doc_id:${documentId}]`,
                      'document',
                      '📝',
                      '/student/documents'
                    ]
...
router.delete('/:id', authenticate, async (req, res) => {
  try {
    ...
    // If this is a submission document, update the weekly_submission status
    if (document.type === 'submission' && document.student_id && document.week_number) {
      await pool.query(
        `UPDATE weekly_submissions 
         SET status = 'pending', file_path = NULL, file_name = NULL, submission_date = NULL
         WHERE student_id = ? AND week_number = ?`,
        [document.student_id, document.week_number]
      );
      
      // Recalculate and update student progress
      const TOTAL_WEEKS = 6; // This should be dynamic based on weekly_tasks, but keeping for now
...
// Add feedback/comment to a document (independent of document status)
router.post('/:id/feedback', authenticate, [
  ...
  try {
    ...
    try {
      ...
      // Insert feedback without changing document status (status is just for the review record)
      const [result] = await pool.query(
    ...
    // Don't update document status when adding feedback - feedback is independent
    // Document status should be updated separately via approve or correction endpoints
    console.log('Feedback added without changing document status');
